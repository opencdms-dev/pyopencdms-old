...
def group_observation_final_data(db_session: Session, first_obs_date: bool = True):
    ...
    return (
        db_session.query(
            ...
            func.MIN(subquery.c.obsDate)
            if first_obs_date
            else func.MAX(subquery.c.obsDate),
        )
        .group_by("recordedFrom", "describedBy")
        .all()
    )
...
def get_station_element_data(db_session: Session):
    return (
        ...
        .order_by(Stationelement.beginDate)
        .all()
    )
...
def sync_stationelement_with_observationfinal_sqla(db_session: Session):
    ...
    """
    observationfinal_grouped_rows = group_observation_final_data(db_session)
    station_element_rows = get_station_element_data(db_session)
...
def auto_update_end_date(
    db_session: Session, delay_threshold: datetime.timedelta = None
):
    """
