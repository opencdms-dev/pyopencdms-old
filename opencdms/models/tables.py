...
from sqlalchemy.orm import declarative_base, relationship, backref, mapper
Base = declarative_base()
metadata = Base.metadata

observation_type_table = Table(
...
def start_mappers():
    mapper(models.ObservationType, observation_type_table)
    mapper(models.Feature, features_table)
    mapper(models.Collection, collections_table)
    mapper(models.FeatureType, feature_type_table)
    mapper(models.ObservedProperty, observed_property_table)
    mapper(models.ObservingProcedure, observing_procedure_table)
    mapper(models.RecordStatus, record_status_table)
    mapper(models.Sensor, sensors_table)

    station_mapper = mapper(models.Station, stations_table)
    user_mapper = mapper(models.User, users_table)
    observation_mapper =  mapper(models.Observation, observations_table,
       properties = {
          "source_station": relationship(station_mapper,\
             backref=backref("observation", lazy=False), lazy=False)
       })
    mapper(models.StationRole, station_roles_table, properties={
        "user": relationship(user_mapper, backref=backref("station_roles", lazy=False),\
             lazy=False),
        "station": relationship(station_mapper, backref=backref("roles", lazy=False), \
            lazy=False)
    })