...
class Equipment(Base):
    ...
    equipment_type_id = Column(
        Integer, nullable=False, comment="Unique identifier for equipment type"
    )
    ...
    eqpt_prct_date = Column(
        DateTime,
        comment="Equipment procurement date, or date of " "calibration",
    )
...
class ObservingSchedule(Base):
    ...
    ob_schd_id = Column(
        Integer, primary_key=True, comment="Unique number of the record"
    )
...
class ObservingSystemInstallation(Base):
    ...
    ob_sys_intl_id = Column(
        Integer, primary_key=True, comment="Unique number for each record"
    )
