...
class Equipment(Base):
    ...
    equipment_cost = Column(
        NUMERIC(6, 2, True), comment="Cost of equipment"
    )
    ...
    eqpt_dspl_rmrk = Column(
        VARCHAR(200), comment="Equipment disposal remark"
    )
...
class ReportingSchedule(Base):
    ...
    id = Column(
        VARCHAR(8),
        comment="Non unique identifier for station",
    )
    id_type = Column(
        VARCHAR(4),
        nullable=False,
    ...
    rpt_schd_rmrk = Column(
        VARCHAR(200), comment="Remark for the record"
    )
...
class Source(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "high_prcn_lat BETWEEN -90.000 AND 90.000"
        ),
        CheckConstraint(
            "high_prcn_lon BETWEEN -179.999 AND 180.000"
        ),
        CheckConstraint(
            " wmo_region_code BETWEEN '1'AND '7'"
        ),
        CheckConstraint("EAST_GRID_REF >= 0"),
        CheckConstraint("NORTH_GRID_REF >= 0"),
        CheckConstraint(
            "SRC_BGN_DATE BETWEEN "
            " TO_DATE('01011677','DDMMYYYY') "
            "AND TO_DATE('31123999','DDMMYYYY')"
        ),
        CheckConstraint(
    ...
    high_prcn_lat = Column(
        NUMERIC(7, 5, True), nullable=False
    )
    high_prcn_lon = Column(
        NUMERIC(8, 5, True), nullable=False
    )
    ...
    rec_st_ind = Column(
        NUMERIC(4, 0, False), nullable=False
    )
    src_bgn_date = Column(
        DateTime,
        comment="\nsource.src_bgn_date "
    ...
    east_grid_ref = Column(
        Integer,
        comment="\nAs a compound with North Grid reference can "
        "indicate a location to within a 100 metre square.\n",
    )
    north_grid_ref = Column(
        NUMERIC(7, 0, False),
        comment="\nAs a compound with east grid reference can "
        "give a location of a site to a 100 m square.\n",
    )
    hydr_area_id = Column(
        NUMERIC(4, 0, False),
        comment="\nHydrometric area identification number."
        " Rainfall stations are located in a hydrometric"
        " area assigned clockwise round the country starting"
        " in North Scotland.\nHydrometric area numbers are in"
        " the HHHh format, where HHH is the original number"
        " allocated in the 1930s, and h is the sub-division "
        " resulting from water authority re-organizations.  "
        " Thus, when h=0 the boundary remains that defined n "
        " the 1930s.\nIn MIDAS, it links an area to sources.\n",
    )
    post_code = Column(VARCHAR(9))
    src_end_date = Column(
        DateTime,
        comment="\nsource.src_end_date is the date"
        " when the station closed\n",
    )
    elevation = Column(
        NUMERIC(6, 2, True),
        comment="\nHeight of ground surface above mean sea level."
        " See also height.\n",
    )
    wmo_region_code = Column(
        CHAR(1),
        comment="\nWMO Code A1 Code table 0161. WMO Regional "
        " Association area in which buoy, drilling rig"
        " or oil- or gas-production platform has been "
        " deployed.\nValues: 1 = Africa, 2 = Asia, "
        " 3 = South America, 4 = North America, 5 = Australasia,"
        " 6 = Europe, 7 = Antactica.\n",
    )
    ...
    drainage_stream_id = Column(
        VARCHAR(4),
        comment="\nDrainage streams or coastal name identification number"
        " to link area to source information. New item but drainage"
        " stream information from ML.HYDROSET\n",
    )
    src_upd_date = Column(
        DateTime, comment="\nsource_updated_date\n"
    )
    ...
    src_geom = Column(
        JSON, comment="SRID 8307 geometry (WGS84 lat/lon)"
    )
...
class StationReportElement(Base):
    ...
    met_element_id = Column(
        NUMERIC(5, 0, False), nullable=False
    )
...
class Deployment(Base):
    ...
    __table_args__ = (
        ...
        {
            "comment": "An instance of a piece of equipment being \
                        deployed for a purpose. Provision is made \
                        for the situation where only the type of equipment\
                        is known, by carrying Instrument Type ID as a \
                        foreign key, and making the relationship with \
                        the Equipment entity optional. Need to ensure\
                        that, if a specific piece of equipment is recorded\
                        (via Equipment ID), the attribute Equipment Type ID \
                        is automatically set to the value of the\
                        corresponding attribute in Equipment."
        },
    )
    ...
    id = Column(
        VARCHAR(8),
        comment="Identifier associated with station",
    )
    id_type = Column(
        VARCHAR(4),
        nullable=False,
    ...
    met_office_eqpt_flag = Column(
        CHAR(1),
        nullable=False,
        comment=" Flag describing whether Met Office"
        " owns equipment or not (T or F)",
    )
    ob_sys_name = Column(
        VARCHAR(12),
        index=True,
    ...
    elevation = Column(
        NUMERIC(6, 2, True),
        comment="Elevation of deployment",
    )
    deployment_remark = Column(
        VARCHAR(250), comment="Remark about deployment"
    )
    lat_wgs84 = Column(
        NUMERIC(7, 5, True),
    ...
    ipr_owner = Column(
        VARCHAR(12),
        comment=" Code identifying who owns the intellectual"
        " property rights of the deployed equipment.",
    )
...
# E       sqlalchemy.exc.ProgrammingError: (psycopg2.errors.UndefinedColumn)
#          column "equipment_id" referenced in foreign key
...
#
# /opt/hostedtoolcache/Python/3.8.12/x64/lib/python3.8
# /site-packages/sqlalchemy/engine/default.py:717: ProgrammingError
#
...
class Inspection(Base):
    ...
    __table_args__ = (
        CheckConstraint("SRC_ID >= 0"),
        {
            "comment": "Provides a record of inspections carried\
                        out at stations and related to the \
                        midas.source table"
        },
    )
    ...
    inspection_remark = Column(
        VARCHAR(700), comment="Remark on inspection"
    )
...
class ObservingSchedule(Base):
    ...
    stn_rpt_elem_id = Column(
        ForeignKey(
            "station_report_element.stn_rpt_elem_id",
            ondelete="CASCADE",
        ),
        nullable=False,
...
class SrcCapability(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "FIRST_ONLINE_OB_YR BETWEEN 1738 AND 3999"
        ),
        CheckConstraint(
            "SRC_CAP_END_DATE >= SRC_CAP_BGN_DATE"
        ),
        CheckConstraint(
            "SRC_CAP_END_DATE >= SRC_CAP_BGN_DATE"
        ),
        ...
        CheckConstraint(
            "SRC_CAP_END_DATE BETWEEN TO_DATE('01011677','DDMMYYYY')"
            " AND TO_DATE('31123999','DDMMYYYY')  "
        ),
        CheckConstraint("REC_ST_IND >= 0001"),
        CheckConstraint(
            "SRC_CAP_BGN_DATE BETWEEN TO_DATE('16770101','YYYYMMDD') "
            "AND TO_DATE('39991231','YYYYMMDD')"
        ),
        CheckConstraint(
            "SUBSTR(RCPT_METHOD_NAME,1,8) = 'ID XREF '"
        ),
        Index(
            "c_srccap_end_date",
            "id",
        ...
        {
            "comment": "Midas.Src_Capability table defines which types \
                       of observation  (met domain) a source is capable\
                       of producing (and MIDAS will store),\
                       e.g. London/Gatwick is capable of producing synops and\
                       NCMs, while Southend is only capable of producing\
                       metars.\nSome stations, e.g. Beaufort Park, use more\
                       than one identifier of the same type, e.g. WMO number\
                       03693 for manned observations and 03694 for SAMOS,\
                       therefore there will be two capabilities for this\
                       source.\nChanges over time are recorded using dates.\
                       A source capability is closed when attribute\
                       Src_Cap_End_Date is set to a date that is before\
                       the current date.  If the capability is subsequently\
                       required again by the source, then the record may \
                       either be re-opened, by resetting the Src_Cap_End_Date\
                       or by creating a new record.\nA capability is not \
                       automatically created upon receipt of a new source\
                       or new meteorological domain.  A source capability\
                       can be deleted when it is open.\nRcpt_method_name\
                       is currently used to store ID cross-references,\
                       while communication_method_code is required by the\
                       MetaData project.\nMidasUpd.Src_Capability is the\
                       corresponding updatable view, with \
                      one-for-one projection of the columns.  To prevent \
                      accidental deletion of CLIMAT rows, delete privilege \
                      is not available for this view.\
                      MidasUpd.Delete_Src_Capability allows delete privilege \
                      on rows other than CLIMAT ones.\
                      MidasUpd.Clm_Src_Capability is an updatable view, \
                      restricted to CLIMAT rows.\nMidasVu.Src_Capability\
                      is a non-updatable view, with one-for-one projection \
                      of the columns.\nWhen the Src_Capability is valid,\
                      but MIDAS has no data, Rec_St_Ind = 2000.  \
                      These rows are excluded from the MidasVu views."
        },
    )
    ...
    id = Column(
        ...
        primary_key=True,
        nullable=False,
        comment="\nThe value of an identifier for the source. "
        " Eight left-justified identifier characters,"
        " eg 03772, EGGW , etc.  This ID has no meaning "
        " unless interpreted with the associated identifier"
        " type.\nIdentifier has a length of eight characters"
        " to allow for ship call-signs and to achieve alignment."
        " The WMO definition of symbolic letter D....D is Ships"
        " call sign consisting of three or more alphanumeric"
        " characters.  B Fullagar (OPR3) confirms that the"
        " maximum length of a WMO call-sign is seven characters."
        " The GreenPeace ship uses an eight character call-sign,"
        " but OPR truncate this to seven characters.\nSome "
        " identifiers have been re-used, e.g. when a WMO station"
        " number is re-allocated at a later date for a site "
        " that is deemed suitable for synoptic purposes.  Where "
        " this has happened already the oldest use will have"
        " the character Z appended to the identifier, the next "
        " oldest Y, etc, with the over-riding proviso that the"
        " newest version will NOT have an appendage.\n",
    )
    id_type = Column(
        VARCHAR(4),
        primary_key=True,
        nullable=False,
        comment="\nThe name of the type of identifier used "
        " to identify a source,  e.g. WMO, SHIP, OWS,"
        " RAIN, DCNN etc.  See BUFR table B, class 02, 002001.\n",
    )
    met_domain_name = Column(
        VARCHAR(8),
        primary_key=True,
        ...
        "\nThe names of input domains correspond to existing "
        " code forms, but are structured to facilitate sorting, "
        " e.g. SYNOP FM12, SHIP FM13, DRIFTER FM18-IX, CLIMAT UA"
        " FM75-VI, etc.\nNB: Synop is not a unique name for an"
        " input met domain; the domain name must include a FM "
        " session number or date.    Synop pre 1982 is not a "
        " unique name, the name may have changed on several "
        " previous occasions.\nOP3 marine QC have specifically"
        " asked for a met domain of Ship synop - GTS, or similar,"
        " so that they can distinguish GTS ships from VOF ones.  "
        " The names of storage domains will correspond to MIDAS "
        " table names.\n",
    )
    src_cap_bgn_date = Column(
        DateTime,
        primary_key=True,
        nullable=False,
        comment="\nsrc_capability.src_bgn_date is the first date"
        " for which we have observations of the id_ type"
        " / met_domain_name combination.\n",
    )
    src_id = Column(
        ForeignKey("source.src_id"),
        nullable=False,
        comment="\nThis is an identifier for each source of "
        " meterological data within the system. It acts "
        " as a unique identifier when a source may be of"
        "  various types.  It consists of a 6 digit integer,"
        " assigned from a high-number record, and has no "
    ...
    prime_capability_flag = Column(
        Enum("T", "F", name="prime_capability_flag_enum"),
        nullable=False,
        server_default=text("'F'"),
        comment="Single character to indicate if the capability"
        "is the prime one of its type for the specified"
        "station, i.e. the prime daily rainfall capability"
        "for the station.  Valid values are T and F. "
        "For each met_domain, one and only one capability "
        "can be set to prime at one time.  When SAMOS or"
        "similar equipment is trialed at a site, the site may"
        "continue to report using its current ID and"
        "prime_capability_flag = T while the SAMOS uses a new"
        "src_capability and ID with prime_capability_flag = F."
        " When the trial concludes and the SAMOS becomes operational,"
        " the old capability is changed to prime_capability_flag = F"
        " (and is usually closed), while the new"
        " src_capability (and ID) is updated to"
        " prime_capability_flag = T.",
    )
    src_cap_end_date = Column(
        DateTime,
        comment="\nsrc_capability.src_end_date is the last date for"
        " which we have observations of the id_ type /"
        " met_domain_name combination.\n",
    )
    first_online_ob_yr = Column(NUMERIC(4, 0, False))
    db_segment_name = Column(
        VARCHAR(12),
        comment="\nThe name of a database segment,"
        " e.g. source, raindrnl, etc.\n",
    )
    rcpt_method_name = Column(
        VARCHAR(20),
        comment="\nFormerly the method of receiving these"
        " reports from this source, e.g. GTS, metform,"
        " postcard, etc.  This attribute is now used to "
        " cross-reference between IDs in use at a SOURCE."
        " It will be renamed to ID_CROSS_REF at the next"
        " convenient opportunity.\n ",
    )
    data_retention_period = Column(
        NUMERIC(3, 0, False),
        comment="\nCharacter string indicating the period for"
        " which data are retained in MIDAS.\n",
    )
    comm_mthd_id = Column(
        Integer,
        comment="\nAbbreviated code to identify the various"
        " communication methods on which data can be "
...
class SrcCapabilityNodatum(Base):
    ...
    id = Column(
        VARCHAR(8), primary_key=True, nullable=False
    )
    id_type = Column(
        VARCHAR(4), primary_key=True, nullable=False
    )
    met_domain_name = Column(
        VARCHAR(8), primary_key=True, nullable=False
    )
    ...
    src_id = Column(
        ForeignKey("source.src_id"), nullable=False
    )
    rec_st_ind = Column(
        NUMERIC(4, 0, False), nullable=False
    )
    prime_capability_flag = Column(
        Enum("T", "F", name="prime_capability_flag"),
        nullable=False,
    )
...
class StationAuthorityHistory(Base):
    ...
    auth_hist_rmrk = Column(
        VARCHAR(200), comment="Remark on relationship"
    )
...
class StationGeography(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "PRC_MAN_WTHN_10M BETWEEN 0 AND 100"
        ),
        CheckConstraint(
            "PRC_MAN_WTHN_50M BETWEEN 0 AND 100"
        ),
        CheckConstraint(
            "PRC_VEG_WTHN_10M BETWEEN 0 AND 100"
        ),
        CheckConstraint(
            "PRC_VEG_WTHN_50M BETWEEN 0 AND 100"
        ),
        CheckConstraint("SRC_ID >= 0"),
        CheckConstraint(
            "prc_heat_src_wthn_100m between 0 and 100"
        ),
        CheckConstraint(
            "prc_heat_src_wthn_30m between 0 and 100"
        ),
        CheckConstraint(
            "prc_veg_wthn_100m between 0 and 100"
        ),
        CheckConstraint(
            "prc_veg_wthn_30m between 0 and 100"
        ),
        {
            "comment": "Table providing details of the\
                       geography associated with a station."
        },
    )
    ...
    prc_veg_wthn_10m = Column(
        NUMERIC(3, 0, False),
        comment="Percentage vegetation ground cover "
        "greater than 0.3m height within 10m radius",
    )
    prc_man_wthn_50m = Column(
        NUMERIC(3, 0, False),
        comment="Percentage manmade ground cover within 50m radius",
    )
    prc_veg_wthn_50m = Column(
        NUMERIC(3, 0, False),
        comment="Percentage vegetation ground cover greater than"
        " 0.3m height within 50m radius",
    )
    soil_type_id = Column(
        NUMERIC(3, 0, False),
        comment="Unique identifier of soil type at station",
    )
    prc_heat_src_wthn_30m = Column(
        NUMERIC(3, 0, False),
        comment="Percentage of area occupied by sources of"
        " heat within 30m radius",
    )
    prc_veg_wthn_30m = Column(
        NUMERIC(3, 0, False),
        comment="Percentage vegetation ground cover greater "
        "than 0.3m height within 30m radius",
    )
    prc_heat_src_wthn_100m = Column(
        NUMERIC(3, 0, False),
        comment="Percentage of area occupied by sources of"
        " heat within 100m radius",
    )
    prc_veg_wthn_100m = Column(
        NUMERIC(3, 0, False),
        comment="Percentage vegetation ground cover greater"
        " than 0.3m height within 100m radius",
    )
    sfc_cover_class_scheme = Column(
        VARCHAR(20),
        comment="The surface cover classification scheme used "
        "to define the surface cover",
    )
    sfc_cover_less_than_100m = Column(
        VARCHAR(20),
        comment="The type of surface over a radius of the"
        " station of less then 100m",
    )
    sfc_cover_100m_3km = Column(
        VARCHAR(20),
        comment="The type of surface over a radius of the"
        " station between 100m and 3km",
    )
    sfc_cover_3km_100km = Column(
        VARCHAR(20),
        comment="The type of surface over a radius of the "
        "station between 100m and 3km",
    )
...
class StationStatu(Base):
    ...
    status_bgn_date = Column(
        DateTime,
        primary_key=True,
        nullable=False,
        comment="Begin date for status record",
    )
    status_end_date = Column(
        DateTime,
        nullable=False,
...
class DeploymentDetail(Base):
    ...
    __table_args__ = {
        "comment": "The details of attributes associated\
                    with deployments of equipment. related to the \
                    deplyment and the deployment attribute tables."
    }
...
class EqptCalibCoeff(Base):
    ...
    eqpt_type_calib_coeff_id = Column(
        Integer, nullable=False
    )
    eqpt_calib_id = Column(
        ForeignKey("equipment_calibration.eqpt_calib_id"),
        nullable=False,
    )
...
class InspectionDetail(Base):
    ...
    __table_args__ = {
        "comment": "Related to the inspection table and\
                   provides the inspection details (the results)\
