...
class EquipmentCalibration(Base):
    ...
    eqpt_calib_date = Column(
        DateTime, nullable=False, comment="Date on which calibration was carried out"
    )
    calib_mthd_code = Column(
        VARCHAR(4), nullable=False, comment="Code for method of calibration"
    )
    eqpt_calib_next_due_date = Column(
        DateTime, comment="Date on which next calibration is due"
    )
    eqpt_calib_name = Column(
        VARCHAR(28), comment="Name of person carrying out calibration"
    )
...
class SrcCapabilityNodatum(Base):
    ...
    rec_st_ind = Column(NUMERIC(4, 0, False), nullable=False)
    prime_capability_flag = Column(
        Enum("T", "F", name="prime_capability_flag"), nullable=False
    )
...
class StationGeography(Base):
    ...
    pr_geog_type_code = Column(
        VARCHAR(8), nullable=False, comment="Primary geography type at station"
    )
    scdy_geog_type_code = Column(
        VARCHAR(8), comment="Secondary geography type at station"
    )
    site_type_code = Column(
        VARCHAR(20), nullable=False, comment="Unique code of site type"
    )
    land_use_type_id = Column(
        NUMERIC(2, 0, False), nullable=False, comment="Unique identifier of land use"
    )
    grnd_slpe_dsc = Column(
        VARCHAR(20), comment="Description of ground slope (Level, Simple Complex)"
    )
    grnd_slpe_up_dir = Column(
        NUMERIC(3, 0, False), comment="Upward direction of ground slope (0-360)"
    )
    ...
    soil_type_id = Column(
        NUMERIC(3, 0, False), comment="Unique identifier of soil type at station"
    )
...
class StationHistory(Base):
    ...
    event_date = Column(
        DateTime, primary_key=True, nullable=False, comment="Date of event"
    )
    event_text = Column(
        VARCHAR(150), nullable=False, comment="Textual description of event"
    )
    event_result_text = Column(
        VARCHAR(150), nullable=False, comment="Any result of the event"
    )
...
class StationInventoryStatu(Base):
    ...
    invt_sts_rmrk = Column(
        VARCHAR(200), comment="Any remarks applicable to this record"
    )
...
class StationStatu(Base):
    ...
    status_end_date = Column(
        DateTime, nullable=False, comment="End date for status record"
    )
    role_name_code = Column(
        VARCHAR(8), index=True, comment="Role name associated with status record"
    )
...
class InspectionDetail(Base):
    ...
    insp_detl_id = Column(
        NUMERIC(8, 0, False), primary_key=True, comment="Unique identifier of record"
    )
