...
class CodesSimple(Base):
    ...
    code_type = Column(
        String(40),
        nullable=False,
        comment="Character code type",
    )
    code = Column(String(40), nullable=False)
    description = Column(
        String(400), comment="Description of code"
    )
    change_user = Column(
        String(10), comment="User of last change"
    )
    change_datetime = Column(
        DateTime, comment="Timestamp of last change"
    )
    insert_datetime = Column(
        DateTime,
        nullable=False,
        comment="Timestamp of insert",
    )
...
class Equipment(Base):
    ...
    __table_args__ = {
        "comment": "Stores equipment master information."
    }
    ...
    comments = Column(
        String(1000), comment="Comments for equipment"
    )
    version = Column(
        String(50), comment="Version of equipment"
    )
...
class GuiUser(Base):
    ...
    css_filename = Column(
        String(120), comment="Name of style sheet selected"
    )
    layout = Column(
        CHAR(4),
        autoincrement=True,
...
class IngestMonitor(Base):
    ...
    username = Column(
        String(20),
        nullable=False,
        comment="User that started ingest",
    )
    ip_addr = Column(
        String(20),
        comment="Client IP address where ingest was invoked from",
    )
    filename = Column(
        String(200),
        comment="File name of file being ingested",
    )
    ingest_start = Column(
        DateTime,
        nullable=False,
    ...
    file_recs = Column(
        Integer, comment="Count of all input file records"
    )
    ingested_recs = Column(
        Integer,
        comment="Count of all ingested input file records",
    )
...
class KeySetting(Base):
    ...
    profile = Column(
        String(20), nullable=False, comment="profile name"
    )
    obs_type = Column(
        String(20),
        comment="obsservation type: daily, subdaily",
    )
    element = Column(
        String(120), comment="observation element"
    )
    default_unit = Column(
        String(20),
        comment="Default unit in key entry forms",
    )
    disable_flag = Column(
        CHAR(1), comment="Y=disable in entry forms"
    )
...
class LandUse(Base):
    ...
    land_use_code = Column(
        ...
        nullable=False,
        unique=True,
        comment="Land usage code",
    )
    priority = Column(
        Integer,
        nullable=False,
        comment="Land usage priority",
    )
    description = Column(
        String(100), comment="Land use description"
    )
...
class ObsAudit(Base):
    ...
    table_name = Column(
        String(100),
        comment="Observation table where data is changed",
    )
    row_id = Column(
        Integer, comment="Row id of changed data"
    )
    column_name = Column(
        String(100), comment="Column that has been changed"
    )
    column_value = Column(String(4000))
    change_user = Column(
        String(20), comment="User performing the change"
    )
    datetime = Column(
        DateTime(True), comment="Datetime of change"
    )
...
class ObsAverage(Base):
    ...
    __table_args__ = (
        ...
        {
            "comment": "Normals and other monthly long \
                       term averages of observations."
        },
    )
    ...
    month = Column(
        SmallInteger,
        nullable=False,
        comment="Month of averages",
    )
    name = Column(
        String(60),
        nullable=False,
        comment="Name of this set of averages",
    )
    active_normal = Column(CHAR(1))
    from_date = Column(
        Date,
        nullable=False,
        comment="Start date of observations in this average set",
    )
    to_date = Column(
        Date,
        nullable=False,
        comment="End date of observations in this average set",
    )
    station_pres = Column(
        Numeric(7, 1),
    ...
    msl_pres = Column(
        Numeric(7, 1),
        comment="Average MSL Pressure: from obs_subdaily.msl_pres",
    )
    air_temp = Column(
        Numeric(7, 1),
        comment="Average air temp: from obs_subdaily.air_temp",
    )
    max_air_temp = Column(
        Numeric(7, 1),
        comment="Average max air temp: from obs_daily.max_air_temp",
    )
    min_air_temp = Column(
        Numeric(7, 1),
        comment="Average min air temp: from obs_daily.min_air_temp",
    )
    vapour_pres = Column(
        Numeric(7, 1),
        comment="Average vapour pressure: from obs_subdaily.dew_point",
    )
    rainfall = Column(
        Numeric(7, 1),
        comment="Avg Total monthly rainfall. from obs_daily.rain_24h",
    )
    rain_days = Column(
        SmallInteger,
    ...
    missing_station_pres = Column(
        SmallInteger,
        comment="Number of years missing from the record"
                " of normal station pressure",
    )
    ...
    missing_max_min = Column(
        SmallInteger,
        comment="Number of years missing from the record "
                "of normal daily max or min air temp",
    )
    missing_vapour_pres = Column(
        SmallInteger,
        comment="Number of years missing from the "
                "record of normal daily vapour pres",
    )
    ...
    missing_sun_hours = Column(
        SmallInteger,
        comment="Number of years missing from the record "
                "of normal daily sunshine hours",
    )
...
class ObsClicomElementMap(Base):
    ...
    __table_args__ = {
        "comment": "Mapping Clicom Codes to CLDB table, column"
    }
    ...
    column_type = Column(
        String(4),
        server_default=text("'num'::character varying"),
    )
...
class ObsMonthly(Base):
    ...
    __table_args__ = (
        ...
        {
            "comment": "Stores monthly data not available\
                        as daily or subdaily"
        },
    )
    ...
    lsd = Column(
        Date,
        nullable=False,
        comment="Local System Year and Month",
    )
    data_source = Column(
        CHAR(2),
        comment="Code for data source, see "
                "codes_simple for code_type=DATA_SRC",
    )
    insert_datetime = Column(
        DateTime, comment="Date/Time row is inserted"
    )
    change_datetime = Column(
        DateTime, comment="Date/Time row is updated"
    )
    change_user = Column(
        String(20), comment="User who added/changed row"
    )
    qa_flag = Column(CHAR(1), comment="QA flag for row Y/N")
    comments = Column(String(1000), comment="User comments")
    dly_max_rain = Column(
        Numeric(8, 1),
        comment="Highest Daily precipitation mm",
    )
    dly_max_rain_inches = Column(
        Numeric(8, 1),
        comment="Highest Daily precipitation (inches to .001)",
    )
    dly_max_rain_date = Column(
        String(120),
        comment="Date(s) of highest rain as dd,dd,dd,...",
    )
    dly_max_rain_qa = Column(CHAR(2))
    max_max_air_temp = Column(
        Numeric(7, 1),
        comment="Highest daily maximum air temp (C to 0.1)",
    )
    max_max_air_temp_f = Column(
        Numeric(7, 1),
        comment="Highest daily maximum air temp (F to 0.1)",
    )
    max_max_air_temp_qa = Column(CHAR(2))
    max_max_air_temp_date = Column(
        String(120),
        comment="Date(s) of highest max air temp as dd,dd,dd,...",
    )
    min_min_air_temp = Column(
        Numeric(7, 1),
        comment="Lowest daily minimum air temp (C to 0.1)",
    )
    min_min_air_temp_f = Column(
        Numeric(7, 1),
        comment="Lowest daily minimum air temp (F to 0.1)",
    )
    min_min_air_temp_qa = Column(CHAR(2))
    min_min_air_temp_date = Column(
        String(120),
        comment="Date(s) of lowest daily min as dd,dd,dd,...",
    )
    min_min_ground_temp = Column(
        Numeric(7, 1),
        comment="Lowest minimum daily ground temp (C to 0.1)",
    )
    min_min_ground_temp_f = Column(
        Numeric(7, 1),
        comment="Lowest minimum daily ground temp (F to 0.1)",
    )
    ...
    mn_air_temp = Column(
        Numeric(7, 1),
        comment="Mean air temperature (C to 0.1)",
    )
    mn_air_temp_f = Column(
        Numeric(7, 1),
        comment="Mean air temperature (F to 0.1)",
    )
    mn_air_temp_qa = Column(CHAR(2))
    mn_max_air_temp = Column(
        Numeric(7, 1),
        comment="Mean of maximum daily air temp (C to 0.1)",
    )
    mn_max_air_temp_f = Column(
        Numeric(7, 1),
        comment="Mean of maximum daily air temp (F to 0.1)",
    )
    mn_max_air_temp_qa = Column(CHAR(2))
    mn_min_air_temp = Column(
        Numeric(7, 1),
        comment="Mean of minimum daily air temp (C to 0.1)",
    )
    mn_min_air_temp_f = Column(
        Numeric(7, 1),
        comment="Mean of minimum daily air temp (F to 0.1)",
    )
    mn_min_air_temp_qa = Column(CHAR(2))
    mn_wet_bulb_temp = Column(
        Numeric(7, 1),
        comment="Mean wet bulb temperature (C to 0.1)",
    )
    mn_wet_bulb_temp_f = Column(
        Numeric(7, 1),
        comment="Mean wet bulb temperature (F to 0.1)",
    )
    mn_wet_bulb_temp_qa = Column(CHAR(2))
    mn_min_ground_temp = Column(
        Numeric(7, 1),
        comment="Mean of minimum daily ground temp (C to 0.1)",
    )
    mn_min_ground_temp_f = Column(
        Numeric(7, 1),
        comment="Mean of minimum daily ground temp (F to 0.1)",
    )
    mn_min_ground_temp_qa = Column(CHAR(2))
    mn_asread_pres = Column(
        Numeric(7, 1),
        comment="Mean as read pressure (hPa to 0.1)",
    )
    mn_asread_pres_inches = Column(
        Numeric(8, 3),
        comment="Mean as read pressure (inches to 0.001)",
    )
    mn_asread_pres_mmhg = Column(
        Numeric(7, 2),
        comment="Mean as read pressure (mmHg to 0.01)",
    )
    mn_asread_pres_qa = Column(CHAR(2))
    mn_msl_pres = Column(
        Numeric(7, 1),
        comment="Mean MSL pressure (hPa to 0.1)",
    )
    mn_msl_pres_inches = Column(
        Numeric(8, 3),
        comment="Mean MSL pressure (inches to 0.001)",
    )
    mn_msl_pres_mmhg = Column(
        Numeric(7, 2),
        comment="Mean MSL pressure (mmHg to 0.01)",
    )
    mn_msl_pres_qa = Column(CHAR(2))
    mn_station_pres = Column(
        Numeric(7, 1),
        comment="Mean station level pressure (hPa to 0.1)",
    )
    mn_station_pres_inches = Column(
        Numeric(8, 3),
        comment="Mean station level pressure (inches to 0.001)",
    )
    mn_station_pres_mmhg = Column(
        Numeric(7, 2),
        comment="Mean station level pressure (mmHg to 0.01)",
    )
    mn_station_pres_qa = Column(CHAR(2))
    mn_vapour_pres = Column(
        Numeric(7, 1),
        comment="Mean Vapour Pressure (hPa to 0.1)",
    )
    mn_vapour_pres_inches = Column(
        Numeric(8, 3),
        comment="Mean Vapour Pressure (Inches to 0.001)",
    )
    mn_vapour_pres_mmhg = Column(
        Numeric(7, 2),
        comment="Mean Vapour Pressure (mmHg to 0.01)",
    )
    mn_vapour_pres_qa = Column(CHAR(2))
    mn_evaporation = Column(
        Numeric(4, 1),
        comment="Mean of daily evaporation (mm to 0.1)",
    )
    mn_evaporation_inches = Column(
        Numeric(6, 3),
        comment="Mean of daily evaporation (Inches to 0.001)",
    )
    mn_evaporation_qa = Column(CHAR(2))
    mn_rel_humidity = Column(
        Numeric(4, 1),
        comment="Mean Relative Humidity (% to 0.1)",
    )
    mn_rel_humidity_qa = Column(CHAR(2))
    mn_sun_hours = Column(
        Numeric(4, 2),
        comment="Mean of daily bright sunshine (hours to 0.01)",
    )
    mn_sun_hours_qa = Column(CHAR(2))
    mn_tot_cloud_oktas = Column(
        Numeric(1, 0),
        comment="Mean of Daily Total Cloud Amt (Octas 0-9)",
    )
    mn_tot_cloud_tenths = Column(
        Numeric(2, 0),
        comment="Mean of Daily Total Cloud Amt (Tenths 0-10)",
    )
    mn_tot_cloud_qa = Column(CHAR(2))
    tot_evaporation = Column(
        Numeric(8, 1),
        comment="Total Monthly evaporation (mm to 0.1)",
    )
    tot_evaporation_inches = Column(
        Numeric(9, 3),
        comment="Total Monthly evaporation (Inches to 0.001)",
    )
    tot_evaporation_qa = Column(CHAR(2))
    tot_rain = Column(
        Numeric(8, 1),
        comment="Total monthly precipitation  (mm to 0.1)",
    )
    tot_rain_inches = Column(
        Numeric(9, 3),
        comment="Total monthly precipitation  (Inches to 0.001)",
    )
    tot_rain_qa = Column(CHAR(2))
    tot_rain_days = Column(
        Numeric(4, 0), comment="Number of rain days"
    )
    tot_rain_days_qa = Column(CHAR(2))
    tot_rain_percent = Column(
        Numeric(4, 0),
        comment="Percentage complete (not missing)"
                " of daily records for month",
    )
...
class ObsSubdailySoilTemp(Base):
    ...
    __table_args__ = (
        Index(
            "fki_obs_subdaily_soil_temps_subdaily_id_fkey",
            "sub_daily_id",
        ),
    ...
    soil_depth = Column(
        Numeric(5, 0),
        nullable=False,
        comment="Soil depth in cm",
    )
    soil_temp = Column(
        Numeric(7, 1), comment="Soil Temperature (C to 0.1)"
    )
    soil_temp_f = Column(
        Numeric(7, 1), comment="Soil Temperature (F to 0.1)"
    )
    soil_temp_qa = Column(
        CHAR(2), comment="Quality Code for soil_temp"
    )
...
class ObscodesCloudAmtConv(Base):
    ...
    code_0501 = Column(
        CHAR(1), comment="WMO Code 0501 code form"
    )
    code_2700 = Column(
        CHAR(1), comment="WMO Code 2700 code form"
    )
    code_bft = Column(String(10), comment="Beaufort code")
    tenths = Column(
        String(5),
        comment="Tenths (Can be multiple values comma separated)",
    )
...
class ObscodesCloudConv1677(Base):
    ...
    __table_args__ = {
        "comment": "Cloud Height conversions for WMO 1677"
    }
    ...
    low_feet = Column(
        Numeric(7, 0), comment="Lower bound in feet"
    )
    low_meters = Column(
        Numeric(7, 0), comment="Lower bound in M"
    )
    change_user = Column(
        String(10), comment="User of last change"
    )
    ...
    insert_datetime = Column(
        DateTime, comment="Timestamp of insert"
    )
...
class ObscodesCloudHtConv(Base):
    ...
    low_feet = Column(
        Numeric(9, 0), comment="Lower bound in feet"
    )
    high_feet = Column(
        Numeric(9, 0), comment="Upper bound in Feet"
    )
    low_meters = Column(
        Numeric(7, 0), comment="Lower Bound in M"
    )
    high_meters = Column(
        Numeric(7, 0), comment="Upper Bound in M"
    )
    change_user = Column(
        String(10), comment="User of last change"
    )
...
class ObscodesCloudTypeConv(Base):
    ...
    wmo_table = Column(
        CHAR(4),
        comment="WMO Table no (0513 Low, 0515 Mid, 0509 High)",
    )
    layer = Column(
        String(4), comment="Cloud Layer: Low, Mid, High"
    )
    types = Column(
        String(10), comment="Acceptable Cloud types"
    )
    change_user = Column(
        String(10), comment="User of last change"
    )
...
class ObscodesVisibility(Base):
    ...
    __table_args__ = {
        "comment": "Visibility conversions: Aero,\
                    non-Aero, Km, yards. WMO 4300"
    }
    ...
    non_aero_scale = Column(
        String(2),
        nullable=False,
        comment="Visibility non-Aero scale",
    )
    distance_km = Column(
        Numeric(5, 2), comment="Distance in Km"
    )
    distance_yards = Column(
        Numeric(7, 0), comment="Distance in Yards"
    )
...
class ObscodesWindDir(Base):
    ...
    __table_args__ = {
        "comment": "Wind Direction conversions: \
                   Compass points to degrees"
    }
    ...
    compass = Column(
        String(6),
        nullable=False,
        comment="Compass points: NNE, SE, CLM,",
    )
    degrees = Column(
        Numeric(3, 0), comment="Degrees (0-360)"
    )
    low_degrees = Column(
        Numeric(5, 2), comment="Lower bound in degrees (>)"
    )
    high_degrees = Column(
        Numeric(5, 2), comment="Upper bound in degrees (<=)"
    )
    change_user = Column(
        String(10), comment="User of last change"
    )
...
class ObscodesWindSpeed(Base):
    ...
    code_bft = Column(
        String(2), nullable=False, comment="Beaufort code"
    )
    ms = Column(Numeric(5, 2), comment="M/S")
    low_ms = Column(
        Numeric(5, 2), comment="Lower bound in M/S"
    )
    high_ms = Column(
        Numeric(5, 2), comment="Upper bound in M/S"
    )
    low_knots = Column(
        Numeric(5, 2), comment="Lower bound in Knots"
    )
    high_knots = Column(
        Numeric(5, 2), comment="Upper bound in Knots"
    )
    change_user = Column(
        String(10), comment="User of last change"
    )
...
class ObscodesWx(Base):
    ...
    code = Column(
        String(2), nullable=False, comment="WMO 4677 code"
    )
    name = Column(String(40), comment="Name of phenomenon")
    description = Column(
        String(200), comment="Description of phenomenon"
    )
    change_user = Column(
        String(10), comment="User of last change"
    )
...
class ObsconvFactor(Base):
    ...
    mult_factor = Column(
        Numeric(7, 4),
        nullable=False,
        comment="Conversion factor. Multiplied by From.",
    )
    post_sum = Column(
        Numeric(7, 4),
        comment="Value to add after multiplying conversion factor.",
    )
    change_user = Column(
        String(10), comment="User of last change"
    )
...
class Pivot(Base):
    __tablename__ = "pivot"
    __table_args__ = {
        "comment": "Utility table of sequential integers"
    }
...
class SoilType(Base):
    ...
    soil_type = Column(
        String(10), nullable=False, comment="Soil type code"
    )
    description = Column(
        String(50), comment="Soil type description"
    )
...
class StationAuditType(Base):
    ...
    audit_type = Column(
        ...
        nullable=False,
        unique=True,
        comment="Audit type code",
    )
    description = Column(
        String(50), comment="Description of audit type"
    )
    system_type = Column(
        CHAR(1),
        nullable=False,
...
class StationStatu(Base):
    ...
    status = Column(
        String(10), nullable=False, comment="Status Code"
    )
    description = Column(
        String(50), comment="Description of status"
    )
...
class StationTimezone(Base):
    ...
    __table_args__ = {
        "comment": "Stores time zone that stations can be in."
    }
    ...
    description = Column(
        String(50), comment="Description of time zone"
    )
...
class StationType(Base):
    ...
    __table_args__ = {
        "comment": "Stores allowed values for stations.type_id"
    }
    ...
    station_type = Column(
        String(10),
        nullable=False,
        comment="Station type code",
    )
    description = Column(
        String(50), comment="Station type description"
    )
...
class SurfaceType(Base):
    ...
    surface_type = Column(
        String(10),
        nullable=False,
        comment="Surface type code",
    )
    description = Column(
        String(50), comment="Surface type description"
    )
...
class UserSession(Base):
    ...
    username = Column(
        String(20), nullable=False, comment="Login username"
    )
    ...
    end_timestamp = Column(
        DateTime, comment="End of session"
    )
    logout_flag = Column(CHAR(1))
    timeout_flag = Column(
        CHAR(1), comment="Session ended by timeout"
    )
    killed_flag = Column(
        CHAR(1), comment="Session ended by admin kill"
    )
...
class Station(Base):
    ...
    __table_args__ = (
        Index(
            "fki_stations_country_code_fkey", "country_code"
        ),
        Index("fki_stations_time_zone_fkey", "time_zone"),
        Index(
            "fki_stations_soil_type_id_fkey", "soil_type"
        ),
        Index("fki_stations_status_id_fkey", "status_id"),
        Index(
            "fki_stations_surface_type_id_fkey",
            "surface_type",
        ),
        Index(
            "fki_stations_land_use_0_id_fkey", "lu_0_100m"
        ),
        Index(
            "fki_stations_land_use_100_id_fkey",
            "lu_100m_1km",
        ),
        Index(
            "fki_stations_land_use_1km_id_fkey",
            "lu_1km_10km",
        ),
    ...
    country_code = Column(
        ForeignKey("station_countries.iso_code")
    )
    ...
    critical_river_height = Column(
        Numeric(7, 3),
        comment="Critical River height (eg. Flood level) in M",
    )
    ...
    location_epsg = Column(
        ForeignKey(
            "spatial_ref_sys.srid", onupdate="CASCADE"
        )
    )
    ...
    land_use = relationship(
        "LandUse",
        primaryjoin="Station.lu_0_100m == LandUse.id",
    )
    land_use1 = relationship(
        "LandUse",
        primaryjoin="Station.lu_100m_1km == LandUse.id",
    )
    land_use2 = relationship(
        "LandUse",
        primaryjoin="Station.lu_1km_10km == LandUse.id",
    )
...
class TimezoneDiff(Base):
    ...
    __table_args__ = (
        ...
        {
            "comment": "Stores timezone differences due\
                        to daylight savings"
        },
    )
    ...
    start_timestamp = Column(
        DateTime,
        nullable=False,
        comment="Date time difference starts",
    )
    end_timestamp = Column(
        DateTime,
        nullable=False,
        comment="Date time difference ends",
    )
    tm_zone = Column(
        ForeignKey("station_timezones.tm_zone"),
        nullable=False,
    ...
    tm_diff = Column(
        Numeric(4, 1),
        comment="UTC offset during this period",
    )
...
class ObsAero(Base):
    ...
    lct = Column(
        DateTime,
        comment="Local Clock Time (With Daylight Savings)",
    )
    data_source = Column(
        CHAR(2),
        nullable=False,
        comment="Code for data source (Ref Table??)",
    )
    insert_datetime = Column(
        DateTime,
        nullable=False,
    ...
    change_datetime = Column(
        DateTime, comment="Date/time row is changed"
    )
    change_user = Column(
        String(20), comment="User who added/changed row"
    )
    ...
    comments = Column(String(1000), comment="User comments")
    message_type = Column(
        CHAR(1), comment="M=METAR, S=SPECI"
    )
    wind_dir = Column(
        Numeric(4, 0), comment="Degrees (0-360)"
    )
    wind_dir_qa = Column(CHAR(2))
    wind_speed = Column(
        Numeric(4, 1), comment="Wind Speed (M/s to 0.1)"
    )
    wind_speed_qa = Column(CHAR(2))
    max_gust_10m = Column(
        Numeric(4, 1), comment="Max Wind Speed (M/s to 0.1)"
    )
    max_gust_10m_qa = Column(CHAR(2))
    cavok_or_skc = Column(CHAR(1), comment="C=CAVOK, S=SKC")
    visibility = Column(
        Numeric(7, 3), comment="Km to (0.001)"
    )
    visibility_qa = Column(CHAR(2))
    pres_wea_intensity_1 = Column(
        Numeric(1, 0),
        comment="0=Light, 1=Moderate, 2=Heavy,3=In Vicinity",
    )
    pres_wea_desc_1 = Column(
        CHAR(2), comment="MI,BC,PR,DR,BL,SH,TS,FZ"
    )
    pres_wea_phen_1 = Column(
        String(6),
        comment="DZ,RA,SN,SG,IC,PL,GR,GS, BR, FG, FU,"
                " VA, DU, SA, HZ, PO, SQ, FC, SS, DS "
                "(WMO 4678)",
    )
    ...
    pres_wea_desc_2 = Column(
        CHAR(2), comment="MI,BC,PR,DR,BL,SH,TS,FZ"
    )
    pres_wea_phen_2 = Column(
        String(6),
        comment="DZ,RA,SN,SG,IC,PL,GR,GS, BR, FG, FU, VA, DU,"
                " SA, HZ, PO, SQ, FC, SS, DS (WMO 4678)",
    )
    ...
    pres_wea_desc_3 = Column(
        CHAR(2), comment="MI,BC,PR,DR,BL,SH,TS,FZ"
    )
    pres_wea_phen_3 = Column(
        String(6),
        comment="DZ,RA,SN,SG,IC,PL,GR,GS, BR, FG, FU, VA, DU, SA, HZ, PO, "
                "SQ, FC, SS, DS (WMO 4678)",
    )
    pres_wea_3_qa = Column(CHAR(2))
    cloud_amt_oktas_1 = Column(
        Numeric(2, 0), comment="Oktas (0-9)"
    )
    cloud_amt_code_1 = Column(
        CHAR(3), comment="FEW, SCD, BKN,OVC"
    )
    cloud_amt_1_qa = Column(CHAR(2))
    cloud_type_1 = Column(
        String(2), comment="ST, SC, CU, etc"
    )
    cloud_type_1_qa = Column(CHAR(2))
    cloud_height_code_1 = Column(
        CHAR(3), comment="Code (WMO Code 1690)"
    )
    cloud_height_1_qa = Column(CHAR(2))
    cloud_amt_oktas_2 = Column(
        Numeric(2, 0), comment="Oktas (0-9)"
    )
    cloud_amt_code_2 = Column(
        CHAR(3), comment="FEW, SCD, BKN,OVC"
    )
    ...
    cloud_height_code_2 = Column(
        CHAR(3), comment="Code (WMO Code 1690)"
    )
    cloud_height_2_qa = Column(CHAR(2))
    cloud_amt_oktas_3 = Column(
        Numeric(2, 0), comment="Oktas (0-9)"
    )
    cloud_amt_code_3 = Column(
        CHAR(3), comment="FEW, SCD, BKN,OVC"
    )
    ...
    cloud_height_code_3 = Column(
        CHAR(3), comment="Code (WMO Code 1690)"
    )
    cloud_height_3_qa = Column(CHAR(2))
    cloud_amt_oktas_4 = Column(
        Numeric(2, 0), comment="Oktas (0-9)"
    )
    cloud_amt_code_4 = Column(
        CHAR(3), comment="FEW, SCD, BKN,OVC"
    )
    ...
    cloud_height_code_4 = Column(
        CHAR(3), comment="Code (WMO Code 1690)"
    )
    cloud_height_4_qa = Column(CHAR(2))
    cloud_amt_oktas_5 = Column(
        Numeric(2, 0), comment="Oktas (0-9)"
    )
    cloud_amt_code_5 = Column(
        CHAR(3), comment="FEW, SCD, BKN,OVC"
    )
    ...
    qnh_inches = Column(
        Numeric(8, 3), comment="Inches to 0.001"
    )
    qnh_qa = Column(CHAR(2))
    rec_wea_desc_1 = Column(
        CHAR(2), comment="MI,BC,PR,BL,SH,TS,FZ"
    )
    rec_wea_phen_1 = Column(
        String(6),
        comment="REFZRA, REFZDZ, RERA, RESN, REGR, "
                "REBLSN,REDS, RESS, RETS, REUP",
    )
    rec_wea_1_qa = Column(CHAR(2))
    rec_wea_desc_2 = Column(
        CHAR(2), comment="MI,BC,PR,BL,SH,TS,FZ"
    )
    ...
    rec_wea_desc_3 = Column(
        CHAR(2), comment="MI,BC,PR,BL,SH,TS,FZ"
    )
    rec_wea_phen_3 = Column(
        String(6),
        comment="REFZRA, REFZDZ, RERA, RESN, REGR, REBLSN,"
                "REDS, RESS, RETS, REUP",
    )
    rec_wea_3_qa = Column(CHAR(2))
    text_msg = Column(
        String(1024), comment="METAR/SPECI msg"
    )
    error_flag = Column(
        Numeric(1, 0), comment="Code (1=Yes, 0=No)"
    )
    remarks = Column(
        String(400),
        comment="Additional Remarks supplied by observer."
                " (Mainly paper docs)",
    )
    remarks_qa = Column(CHAR(2))
    wind_speed_knots = Column(
        Numeric(5, 1), comment="Wind Speed in Knots"
    )
    max_gust_10m_knots = Column(
        Numeric(5, 1), comment="Max Gust >10M Knots"
    )
...
class ObsAw(Base):
    ...
    measure_period = Column(
        SmallInteger,
        comment='Average Period in minutes. "Standard" is (10 min)',
    )
    mn_wind_dir_pt = Column(
        CHAR(3),
        comment='(10 min) mean direction in points. ("ENE")',
    )
    mn_wind_dir_deg = Column(
        SmallInteger,
        comment="(10 min) mean direction in degrees. (0-360)",
    )
    ...
    mn_wind_speed = Column(
        Numeric(7, 1),
        comment="(10 min) mean wind speed (m/s to 0.1)",
    )
    mn_wind_speed_qa = Column(CHAR(2))
    mn_wind_speed_stddev = Column(
        Numeric(7, 1),
        comment="(10 min) mean speed Std Dev (m/s to 0.1)",
    )
    mn_wind_speed_stddev_qa = Column(CHAR(2))
    mn_gust_speed = Column(
        Numeric(7, 1),
        comment="(10 min) mean gust speed (m/s to 0.1)",
    )
    mn_gust_speed_qa = Column(CHAR(2))
    mn_gust_time = Column(
        String(8), comment="Time at end of sample period"
    )
    mn_gust_time_qa = Column(CHAR(2))
    mn_gust_dir_pt = Column(
        CHAR(3),
        comment='Mean Direction of maximum wind speed. ("ENE")',
    )
    mn_gust_dir_deg = Column(
        SmallInteger,
        comment="Mean Direction of maximum wind speed. (0-360)",
    )
    mn_gust_dir_qa = Column(CHAR(2))
    inst_gust_speed = Column(
        Numeric(7, 1),
        comment="Instantaneous gust speed (m/s to 0.1)",
    )
    inst_gust_qa = Column(CHAR(2))
    inst_gust_time = Column(
        String(8),
        comment="Time of instantaneous gust speed (09:30)",
    )
    inst_gust_time_qa = Column(CHAR(2))
    inst_gust_dir_pt = Column(
        CHAR(3),
        comment='Direction of instantaneous gust speed. ("ENE")',
    )
    inst_gust_dir_deg = Column(
        SmallInteger,
        comment="Direction of instantaneous gust speed. (0-360)",
    )
    inst_gust_dir_qa = Column(CHAR(2))
    mn_temp = Column(
        Numeric(7, 1), comment="(10 min) mean. (C to 0.1)"
    )
    mn_temp_qa = Column(CHAR(2))
    mn_temp_subaveraging = Column(
        Numeric(7, 1),
        comment="Sub-averaged temp (C to 0.1)",
    )
    mn_temp_subaveraging_period = Column(
        SmallInteger,
        comment="Sub-averaging period (minutes)",
    )
    mn_temp_subaveraging_qa = Column(CHAR(2))
    max_temp = Column(
        Numeric(7, 1),
        comment="(10 min) maximum. (C to 0.1)",
    )
    max_temp_time = Column(
        String(8),
        comment="Time of max temperature. (hh:mm:ss)",
    )
    max_temp_time_qa = Column(CHAR(2))
    max_temp_qa = Column(CHAR(2))
    min_temp = Column(
        Numeric(7, 1),
        comment="(10 min) minimum. (C to 0.1)",
    )
    min_temp_qa = Column(CHAR(2))
    min_temp_time = Column(
        String(8),
        comment="Time of min temperature.  (hh:mm:ss)",
    )
    min_temp_time_qa = Column(CHAR(2))
    min_grass_temp = Column(
        Numeric(7, 1),
        comment="(10 min) minimum grass temp. (C to 0.1)",
    )
    min_grass_temp_qa = Column(CHAR(2))
    min_grass_temp_time = Column(
        String(8),
        comment="Time of min grass temp.  (hh:mm:ss)",
    )
    min_grass_temp_time_qa = Column(CHAR(2))
    mn_humidity = Column(
        Numeric(4, 1),
        comment="(10 min)average humidity (% to 0.1)",
    )
    mn_humidity_qa = Column(CHAR(2))
    max_humidity = Column(
        Numeric(4, 1),
        comment="(10 min) maximum humidity (% to 0.1)",
    )
    max_humidity_qa = Column(CHAR(2))
    max_humidity_time = Column(
        String(8),
        comment="Time of maximum humidity.  (hh:mm:ss)",
    )
    max_humidity_time_qa = Column(CHAR(2))
    min_humidity = Column(
        Numeric(4, 1),
        comment="(10 min) minimum humidity (% to 0.1)",
    )
    min_humidity_qa = Column(CHAR(2))
    min_humidity_time = Column(
        String(8),
        comment="Time of minimum humidity.  (hh:mm:ss)",
    )
    min_humidity_time_qa = Column(CHAR(2))
    mn_station_pres = Column(
        Numeric(5, 1),
        comment="Average station pressure (hPa to 0.1)",
    )
    mn_station_pres_qa = Column(CHAR(2))
    mn_sea_level_pres = Column(
        Numeric(5, 1),
        comment="Average sea level pressure (hPa to 0.1)",
    )
    mn_sea_level_pres_qa = Column(CHAR(2))
    max_pres = Column(
        Numeric(5, 1),
        comment="Maximum pressure (hPa to 0.1)",
    )
    max_pres_qa = Column(CHAR(2))
    max_pres_time = Column(
        String(8),
        comment="Time of maximum pressure.  (hh:mm:ss)",
    )
    max_pres_time_qa = Column(CHAR(2))
    min_pres = Column(
        Numeric(5, 1),
        comment="Minimum pressure (hPa to 0.1)",
    )
    min_pres_qa = Column(CHAR(2))
    min_pres_time = Column(
        String(8),
        comment="Time of minimum pressure.  (hh:mm:ss)",
    )
    min_pres_time_qa = Column(CHAR(2))
    tot_rain = Column(
        Numeric(6, 1),
        comment="(10 min) total rainfall (mm to 0.1)",
    )
    tot_rain_qa = Column(CHAR(2))
    tot_rain_two = Column(
        Numeric(6, 1),
...
class ObsDaily(Base):
    ...
    data_source = Column(
        CHAR(2),
        nullable=False,
        comment="Code for data source (DATA_SRC)",
    )
    insert_datetime = Column(
        DateTime,
        nullable=False,
    ...
    rain_24h = Column(
        Numeric(6, 1),
        comment="LCT 0900 to 0900 (mm to 0.1)",
    )
    rain_24h_inches = Column(
        Numeric(7, 3),
        comment="LCT 0900 to 0900 (inches to 0.001)",
    )
    rain_24h_period = Column(
        Numeric(2, 0),
        comment="Period of data: Normally 1 day",
    )
    rain_24h_type = Column(
        String(10),
        comment="rain,frost,fog,dew,trace,snow,other, n/a",
    )
    rain_24h_count = Column(
        Numeric(2, 0),
        comment="No of days rain has fallen. Default 1.",
    )
    rain_24h_qa = Column(
        CHAR(2), comment="Quality code for rain_24h"
    )
    max_air_temp = Column(
        Numeric(7, 1),
        comment="Maximum air temperature (0.1C). Standard 0900",
    )
    max_air_temp_f = Column(
        Numeric(7, 1),
        comment="Maximum air temperature (0.1F). Standard 0900",
    )
    max_air_temp_period = Column(
        Numeric(2, 0),
        comment="Period for max air temp (hours). Standard 0900",
    )
    max_air_temp_time = Column(
        String(5),
        comment="Time that max air temp was reached. Standard 0900",
    )
    max_air_temp_qa = Column(
        CHAR(2), comment="Quality code for max_air_temp"
    )
    min_air_temp = Column(
        Numeric(5, 1),
        comment="Minimum air temperature (0.1C). Standard 0900",
    )
    min_air_temp_f = Column(
        Numeric(5, 1),
        comment="Minimum air temperature (0.1F). Standard 0900",
    )
    min_air_temp_period = Column(
        Numeric(2, 0),
        comment="Period for min air temp (hours). Standard 0900",
    )
    min_air_temp_time = Column(
        String(5),
        comment="Time that min air temp was reached. Standard 0900",
    )
    min_air_temp_qa = Column(
        CHAR(2), comment="Quality code for min_air_temp"
    )
    reg_max_air_temp = Column(
        Numeric(7, 1),
        comment="Maximum air temperature (0.1C). Regional.",
    )
    ...
    reg_min_air_temp = Column(
        Numeric(7, 1),
        comment="Minimum air temperature (0.1C). Regional.",
    )
    reg_min_air_temp_qa = Column(
        CHAR(2), comment="Quality code for min_air_temp"
    )
    ground_temp = Column(
        Numeric(5, 1), comment="Ground surface temp (0.1C)"
    )
    ground_temp_f = Column(
        Numeric(5, 1), comment="Ground surface temp (0.1F)"
    )
    ground_temp_qa = Column(
        CHAR(2), comment="Quality code for ground_temp"
    )
    max_gust_dir = Column(
        Numeric(3, 0), comment="Degrees (0-360)"
    )
    max_gust_dir_qa = Column(
        CHAR(2), comment="Quality code for max_gust_dir"
    )
    max_gust_speed = Column(
        Numeric(4, 1),
        comment="Speed of max wind gust M/s (0.1)",
    )
    ...
    max_gust_time = Column(
        String(5), comment="Time of max wind gust"
    )
    ...
    wind_run_lt10 = Column(
        Numeric(6, 2),
        comment="Wind run taken from <10M (evaporation) Km",
    )
    wind_run_lt10_miles = Column(
        Numeric(6, 2),
        comment="Wind run taken from <10M (evaporation) Miles",
    )
    wind_run_lt10_period = Column(
        Numeric(3, 0),
        comment="Period in hours for Wind run <10",
    )
    ...
    wind_run_gt10 = Column(
        Numeric(6, 2),
        comment="Wind run taken from >10M anemometer Km",
    )
    wind_run_gt10_miles = Column(
        Numeric(6, 2),
        comment="Wind run taken from >10M anemometer Miles",
    )
    ...
    evaporation = Column(
        Numeric(4, 1), comment="Evaporation in mm (0.1)"
    )
    evaporation_inches = Column(
        Numeric(5, 3),
        comment="Evaporation in inches (0.001)",
    )
    evaporation_period = Column(
        Numeric(3, 0),
        comment="Period in hours for evaporation",
    )
    evaporation_qa = Column(
        CHAR(2), comment="Quality code for evaporation"
    )
    ...
    sunshine_duration = Column(
        Numeric(3, 1), comment="Decimal Hours to (0.1)"
    )
    sunshine_duration_qa = Column(
        CHAR(2),
        comment="Quality code for sunshine_duration",
    )
    river_height = Column(
        Numeric(5, 1),
        comment="Daily river height reading (0.1M)",
    )
    river_height_in = Column(Numeric(8, 1))
    river_height_qa = Column(
        CHAR(2), comment="Quality code for river_height"
    )
    radiation = Column(
        Numeric(6, 1), comment="Daily radiation Mj/M to 0.1"
    )
    radiation_qa = Column(
        CHAR(2), comment="Quality code for radiation"
    )
    thunder_flag = Column(
        CHAR(1), comment="Y/N for thunder"
    )
    thunder_flag_qa = Column(
        CHAR(2), comment="Quality code for thunder_flag"
    )
    frost_flag = Column(CHAR(1), comment="Y/N for frost")
    frost_flag_qa = Column(
        CHAR(2), comment="Quality code for frost_flag"
    )
    dust_flag = Column(CHAR(1), comment="Y/N for dust")
    dust_flag_qa = Column(
        CHAR(2), comment="Quality code for dust_flag"
    )
    haze_flag = Column(CHAR(1), comment="Y/N for haze")
    haze_flag_qa = Column(
        CHAR(2), comment="Quality code for haze_flag"
    )
    fog_flag = Column(CHAR(1), comment="Y/N for fog")
    fog_flag_qa = Column(
        CHAR(2), comment="Quality code for fog_flag"
    )
    strong_wind_flag = Column(
        CHAR(1), comment="Y/N for strong wind"
    )
    ...
    gale_flag_qa = Column(
        CHAR(2), comment="Quality code for gale_flag"
    )
    hail_flag = Column(CHAR(1), comment="Y/N for hail")
    hail_flag_qa = Column(
        CHAR(2), comment="Quality code for hail_flag"
    )
    snow_flag = Column(CHAR(1), comment="Y/N for snow")
    snow_flag_qa = Column(
        CHAR(2), comment="Quality code for snow_flag"
    )
    lightning_flag = Column(
        CHAR(1), comment="Y/N for lightning"
    )
    lightning_flag_qa = Column(
        CHAR(2), comment="Quality code for lightning_flag"
    )
    shower_flag = Column(CHAR(1), comment="Y/N for shower")
    shower_flag_qa = Column(
        CHAR(2), comment="Quality code for shower_flag"
    )
    rain_flag = Column(CHAR(1), comment="Y/N for rain")
    rain_flag_qa = Column(
        CHAR(2), comment="Quality code for rain_flag"
    )
...
class ObsSubdaily(Base):
    ...
    air_temp = Column(
        Numeric(7, 1),
        comment="Current Air temperature (0.1C)",
    )
    air_temp_f = Column(
        Numeric(7, 1),
        comment="Current Air Temp in Fahrenheit (0.1F)",
    )
    air_temp_qa = Column(
        CHAR(2), comment="Quality Code for air_temp"
    )
    sea_water_temp = Column(
        Numeric(7, 1),
        comment="Current Sea water Temperature (0.1C)",
    )
    sea_water_temp_f = Column(
        Numeric(7, 1),
        comment="Current Sea water Temp in Fahrenheit (0.1F)",
    )
    ...
    wet_bulb = Column(
        Numeric(7, 1),
        comment="Current Wet bulb reading (0.1C)",
    )
    wet_bulb_f = Column(
        Numeric(7, 1),
        comment="Current Wet bulb reading in Fahrenheit (0.1F)",
    )
    wet_bulb_qa = Column(
        CHAR(2), comment="Quality Code for wet_bulb"
    )
    dew_point = Column(
        Numeric(7, 1),
        comment="Current Dew Point Temperature (0.1C)",
    )
    dew_point_f = Column(
        Numeric(7, 1),
        comment="Current Dew Point Fahrenheit (0.1F)",
    )
    dew_point_qa = Column(
        CHAR(2), comment="Quality Code for dew_point"
    )
    rel_humidity = Column(
        Numeric(4, 1),
        comment="Relative humidity  (% to 0.1)",
    )
    rel_humidity_qa = Column(
        CHAR(2), comment="Quality Code for rel_humidity"
    )
    baro_temp = Column(
        Numeric(7, 1),
        comment="Current Barometer Temperature (0.1C)",
    )
    baro_temp_f = Column(
        Numeric(7, 1),
        comment="Current Barometer Fahrenheit (0.1F)",
    )
    baro_temp_qa = Column(
        CHAR(2), comment="Quality Code for baro_temp"
    )
    pres_as_read = Column(
        Numeric(7, 1),
        comment="Pressure as read from barometer (hPa to 0.1)",
    )
    pres_as_read_inches = Column(
        Numeric(8, 3),
        comment="Pressure as read from barometer (Inches to 0.001)",
    )
    pres_as_read_qa = Column(
        CHAR(2), comment="Quality Code for pres_as_read"
    )
    station_pres = Column(
        Numeric(7, 1),
        comment="Station Pressure (hPa to 0.1)",
    )
    station_pres_inches = Column(
        Numeric(8, 3),
        comment="Station Pressure (Inches to 0.001)",
    )
    station_pres_qa = Column(
        CHAR(2), comment="Quality Code for station_pres"
    )
    msl_pres = Column(
        Numeric(7, 1),
        comment="Mean Sea Level Pressure (hPa to 0.1)",
    )
    msl_pres_inches = Column(
        Numeric(8, 3),
        comment="Mean Sea Level Pressure (Inches to 0.001)",
    )
    msl_pres_qa = Column(
        CHAR(2), comment="Quality Code for msl_pres"
    )
    vapour_pres = Column(
        Numeric(7, 1),
        comment="Vapour Pressure (hPa to 0.1)",
    )
    vapour_pres_inches = Column(
        Numeric(8, 3),
        comment="Vapour Pressure (Inches to 0.001)",
    )
    vapour_pres_qa = Column(
        CHAR(2), comment="Quality Code for vapour_pres"
    )
    qnh = Column(
        Numeric(7, 1), comment="Local QNH (hPa to 0.1)"
    )
    qnh_qa = Column(CHAR(2), comment="Quality Code for qnh")
    visibility = Column(
        Numeric(7, 3),
        comment="Visibility in Km (Km to 0.001)",
    )
    visibility_miles = Column(
        Numeric(7, 3),
        comment="Visibility in Miles (Miles to 0.001)",
    )
    visibility_code = Column(CHAR(1))
    visibility_qa = Column(
        CHAR(2), comment="Quality Code for visibility"
    )
    rain_3h = Column(
        Numeric(7, 1),
        comment="3 hours cumulative (mm to 0.1)",
    )
    rain_3h_inches = Column(
        Numeric(7, 3),
        comment="3 hours cumulative (Inches to 0.001)",
    )
    rain_3h_qa = Column(
        CHAR(2), comment="Quality Code for rain_3h"
    )
    rain_3h_hours = Column(
        Numeric(3, 0), server_default=text("3")
    )
    rain_cum = Column(
        Numeric(7, 1),
        comment="Cumulative since 0900 (mm to 0.1)",
    )
    rain_cum_inches = Column(
        Numeric(7, 3),
        comment="Cumulative since 0900 (Inches to 0.001)",
    )
    rain_cum_qa = Column(
        CHAR(2), comment="Quality Code for rain_cum"
    )
    wind_dir = Column(
        Numeric(3, 0),
        comment="10 min Avg Wind direction (degrees 0-360)",
    )
    wind_dir_qa = Column(
        CHAR(2), comment="Quality Code for wind_dir"
    )
    wind_dir_std_dev = Column(
        Numeric(3, 0),
        comment="10 min Avg Wind direction standard deviation",
    )
    ...
    wind_speed = Column(
        Numeric(5, 1),
        comment="10 min Avg Wind Speed (M/S to 0.1)",
    )
    wind_speed_knots = Column(
        Numeric(5, 1),
        comment="10 min Avg Wind Speed (Knots to 0.1)",
    )
    wind_speed_mph = Column(
        Numeric(5, 1),
        comment="10 min Avg Wind Speed (MPH to 0.1)",
    )
    wind_speed_bft = Column(
        CHAR(2),
        comment="10 min Avg Beaufort code for wind speed",
    )
    wind_speed_qa = Column(
        CHAR(2), comment="Quality Code for wind_speed"
    )
    pres_weather_code = Column(
        String(2),
        comment="WMO Code 4677 for present weather.",
    )
    pres_weather_bft = Column(
        String(20),
        comment="Beaufort Code for present weather",
    )
    pres_weather_qa = Column(
        CHAR(2), comment="Quality Code for pres_weather"
    )
    past_weather_code = Column(
        String(2), comment="WMO Code 4561"
    )
    past_weather_bft = Column(
        String(20), comment="Beaufort Code for past weather"
    )
    past_weather_qa = Column(
        CHAR(2), comment="Quality Code for past_weather"
    )
    tot_cloud_oktas = Column(
        SmallInteger,
        comment="Total amount of sky covered by cloud (0-9)",
    )
    tot_cloud_tenths = Column(
        SmallInteger,
        comment="Total amount of sky covered by cloud (0-10)",
    )
    tot_cloud_qa = Column(
        CHAR(2), comment="Quality Code for tot_cloud"
    )
    tot_low_cloud_oktas = Column(
        SmallInteger,
        comment="Total amount of sky covered by Low cloud (0-9)",
    )
    tot_low_cloud_tenths = Column(
        SmallInteger,
        comment="Total amount of sky covered by Low cloud (0-10)",
    )
    ...
    state_of_sea = Column(
        String(2),
        comment="State of Sea (Douglas Scale WMO 3700)",
    )
    state_of_sea_qa = Column(
        CHAR(2), comment="Quality Code for state_of_sea"
    )
    state_of_swell = Column(
        String(2),
        comment="State open sea swell (Douglas Scale WMO 3700)",
    )
    ...
    swell_direction = Column(
        String(3),
        comment="Direction of Swell (16 Compass Points)",
    )
    ...
    sea_level = Column(
        Numeric(5, 3),
        comment="Sea level (M to 0.001) above tide gauge zero",
    )
    sea_level_qa = Column(
        CHAR(2), comment="Quality Code for sea_level"
    )
    sea_level_residual = Column(
        Numeric(5, 3),
        comment="+/- Diff from predicted sea level",
    )
    sea_level_residual_qa = Column(
        CHAR(2),
        comment="Quality Code for sea_level_residual",
    )
    sea_level_resid_adj = Column(
        Numeric(5, 3),
        comment="Adjusted residual (adjusted for pressure)",
    )
    sea_level_resid_adj_qa = Column(
        CHAR(2),
        comment="Quality Code for sea_level_residual_adj",
    )
    radiation = Column(
        Numeric(6, 1), comment="Radiation Mj/M to 0.1"
    )
    ...
    wind_gust_kts = Column(
        Numeric(3, 0), comment="Wind Gust speed Knots"
    )
    wind_gust = Column(
        Numeric(6, 1), comment="Wind Gust speed M/S"
    )
    wind_gust_qa = Column(CHAR(2))
    wind_gust_dir = Column(
        Numeric(3, 0), comment="Wind Gust direction 0-360"
    )
    wind_gust_dir_qa = Column(CHAR(2))
    river_height = Column(
        Numeric(7, 3), comment="River height in M"
    )
    river_height_in = Column(Numeric(8, 1))
...
class ObsUpperAir(Base):
    ...
    pressure = Column(
        Numeric(7, 1),
        nullable=False,
        comment="Pressure (hPa to 0.1)",
    )
    pressure_qa = Column(CHAR(2))
    level_type = Column(
        Numeric(2, 0), comment="Level type (0,1,n)"
    )
    geo_height = Column(Numeric(8, 1), comment="Meters")
    geo_height_qa = Column(CHAR(2))
    air_temp = Column(
        Numeric(7, 1), comment="Temperature (C to 0.1)"
    )
    air_temp_qa = Column(CHAR(2))
    dew_point = Column(
        Numeric(4, 1),
        comment="Dew Point Temperature (C to 0.1)",
    )
    dew_point_qa = Column(CHAR(2))
    wind_direction = Column(
        Numeric(4, 0), comment="Direction (0-360 degrees)"
    )
    wind_direction_qa = Column(CHAR(2))
    wind_speed = Column(
        Numeric(5, 1), comment="Wind Speed (M/s to 0.1)"
    )
    wind_speed_qa = Column(CHAR(2))
...
class StationAudit(Base):
    ...
    __table_args__ = (
        Index(
            "fki_station_audit_audit_type_id_fkey",
            "audit_type_id",
        ),
        Index(
            "fki_station_audit_station_id_fkey",
            "station_id",
        ),
        {
            "comment": "Audit trail of all changes to station Station."
    ...
    station_id = Column(
        ...
        comment="Station ID of audit record",
    )
    datetime = Column(
        DateTime(True),
        comment="Date/Time of event being recorded",
    )
    ...
    description = Column(
        String(1000), comment="Description of audit event"
    )
    event_user = Column(
        String(40),
        comment="User performing the auditable event",
    )
...
class StationClas(Base):
    ...
    __table_args__ = (
        Index(
            "fki_station_class_station_id_fkey",
            "station_id",
        ),
    ...
    type_id = Column(
        ForeignKey("station_types.id"),
        comment="ID of Type for this class",
    )
    description = Column(String(80))
    class_start = Column(
        DateTime,
        comment="Date this class started for the station.",
    )
    class_end = Column(
        DateTime,
        comment="Date this class ended for the station.",
    )
...
class StationContact(Base):
    ...
    __table_args__ = (
        Index(
            "fki_station_contacts_station_id_fkey",
            "station_id",
        ),
        {"comment": "Stores contacts (people) for station"},
    )
...
class StationEquipment(Base):
    ...
    __table_args__ = (
        Index(
            "fki_station_equipment_equipment_id_fkey",
            "equipment_id",
        ),
        Index(
            "fki_station_equipment_station_id_fkey",
            "station_id",
        ),
        {
    ...
    station_id = Column(
        ForeignKey("stations.id"),
        nullable=False,
        comment="ID of station",
    )
    equipment_id = Column(ForeignKey("equipment.id"))
    serial_no = Column(
        String(50), comment="Serial no of equipment"
    )
    asset_id = Column(
        String(50), comment="Asset code of equipment"
    )
    ...
    date_start = Column(
        Date, comment="Date of installation"
    )
    date_end = Column(
        Date, comment="Date of decomissioning or removal"
    )
...
class StationFile(Base):
    ...
    __table_args__ = (
        Index(
            "fki_station_files_station_id_fkey",
            "station_id",
        ),
        {
    ...
    description = Column(
        String(1000), comment="Description of file"
    )
    file_path = Column(
        String(200), comment="Full path to file."
    )
...
class ObsSubdailyCloudLayer(Base):
    ...
    __table_args__ = (
        Index(
            "fki_obs_subdaily_cloud_layers_subdaily_id_fkey",
            "sub_daily_id",
        ),
        {"comment": "Sub Daily surface observations"},
    )
    ...
    layer_no = Column(
        Integer,
        nullable=False,
        comment="layer number. (1,2,3,n)",
    )
    layer_type = Column(String(6), comment="Low, Mid, High")
    cloud_oktas = Column(
        SmallInteger, comment="Cloud  amount in octas (0-9)"
    )
    cloud_tenths = Column(
        SmallInteger,
        comment="Cloud  amount in tenths (0-10)",
    )
    cloud_amt_qa = Column(
        CHAR(2), comment="Quality Code for clout_amt"
    )
    cloud_type = Column(
        String(2), comment="Cloud type, WMO code"
    )
    cloud_type_qa = Column(
        CHAR(2), comment="Quality Code for cloud_type"
    )
    cloud_height = Column(
        Numeric(6, 0),
        comment="Cloud height in Meters (M to 1.0)",
    )
    cloud_height_feet = Column(
        Numeric(7, 0),
        comment="Cloud height in feet (feet to 1.0)",
    )
    cloud_height_qa = Column(
        CHAR(2), comment="Quality Code for cloud_height"
    )
    cloud_dir = Column(
        Numeric(3, 0),
        comment="Cloud movement Direction (0-360)",
    )
    cloud_dir_qa = Column(
        CHAR(2), comment="Quality Code for cloud_dir"
    )
